...
try:
    ...
    st.write('Raw all_dates:', all_dates)
    st.write('Raw all_hours:', all_hours)
    st.write('Raw temp_values:', temp_values)
    ...
    with main_content.container():
        ...
        if not st.session_state['show_scatter']:
            ...
            # Sort by DateTime for correct line plotting
            ...
            ax.set_facecolor('none')  # fully transparent axes
            # Add a heavily rounded, semi-transparent rectangle behind the plot area using axes-relative coordinates
            ...
            # Set axis label colors to super dark for readability
            ...
            # Format x-axis tick labels as 'May 8th, 12:00 PM'
            ...
            temp_df.plot.scatter(x='DateTime', y='Temperature_numeric', ax=ax, color=mpl_heading_color, s=40, zorder=3)
            # Connect the points with a line in chronological order
            ax.plot(temp_df['DateTime'], temp_df['Temperature_numeric'], color=mpl_heading_color, linewidth=2, zorder=2)
            ...
            # Print the final dates and temps arrays used for plotting
            st.write('Plotting DateTime array:', list(temp_df['DateTime']))
            st.write('Plotting Temperature_numeric array:', list(temp_df['Temperature_numeric']))

            # --- Plotly interactive scatter plot for Streamlit ---
            import plotly.graph_objects as go
            plotly_color = mpl_heading_color if mpl_heading_color.startswith('#') else '#1a237e'
            plotly_bg = mpl_card_bg if mpl_card_bg.startswith('#') else '#ffffff'
            plotly_text = mpl_text_color if mpl_text_color.startswith('#') else '#111111'
            # Extract hour and day for distribution
            temp_df['Hour'] = temp_df['DateTime'].dt.hour
            temp_df['Day'] = temp_df['DateTime'].dt.strftime('%b %d')
            temp_df = temp_df.sort_values(['Day', 'Hour'])
            hover_text = [
                f"<b>{dt.strftime('%b %d, %I:%M %p')}</b><br>Temperature: {temp:.1f}°F"
                for dt, temp in zip(temp_df['DateTime'], temp_df['Temperature_numeric'])
            ]
            fig_plotly = go.Figure()
            fig_plotly.add_trace(go.Scatter(
                x=temp_df['Hour'],
                y=temp_df['Temperature_numeric'],
                mode='markers',
                marker=dict(color=plotly_color, size=10),
                hoverinfo='text',
            ...
            fig_plotly.update_layout(
                ...
                font=dict(color=plotly_text, size=16),
            ...
            st.plotly_chart(
                ...
                config={
                    "displaylogo": False,
                    "displayModeBar": False,  # This hides the entire modebar, including the logo
                }
            )

            # Only keep the Back to Dashboard button below the chart
